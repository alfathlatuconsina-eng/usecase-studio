...
class BranchopsUser(Base):
    ...
    # Jatah beberapa cabang tertentu, alternatif dari region_class. Keduanya
    # SALING MENIADAKAN — hanya satu yang boleh terisi, dijaga oleh CHECK
    # ck_bo_users_satu_jatah di branchops/schema.sql. Untuk pengguna yang
    # hanya boleh melihat cabangnya sendiri, atau beberapa cabang dalam
    # satu kota. Tidak dijatah ditulis NULL, tidak pernah larik kosong.
    # ARRAY(Text) supaya cocok dengan TEXT[] di schema.sql — kalau tipenya
...
def _cek_jatah(kelas, kode_list):
    """Periksa sepasang (region_class, branch_codes) untuk seorang pengguna.

    Mengembalikan (kelas, daftar_kode, pesan_error). Paling banyak SATU dari
    keduanya terisi — saling meniadakan. Dua-duanya kosong sah: artinya
    pengguna belum dijatah dan tidak melihat baris apa pun.

    Aturannya tidak ditulis ulang di sini; semuanya dititipkan ke
    scoping.periksa_jatah() supaya jalur ini dan set_jatah() tidak bisa
    berbeda pendapat. Salah ketik ditolak dengan pesan, bukan diterima
    diam-diam — kalau diterima, pengguna mengira sudah dijatah padahal
    layarnya kosong dan tidak ada petunjuk kenapa."""
    from branchops import scoping as _sc
    try:
        return (*_sc.periksa_jatah(kelas, kode_list), None)
        ...
        if "role" in data:
            ...
            if u.role == "admin" and data["role"] != "admin":
                ...
                if admins <= 1:
                    return jsonify({"error": "cannot demote the last admin"}), 400
            u.role = data["role"]
        # Jatah ditulis sebagai SATU kesatuan. Mengirim salah satu kunci saja
        # berarti "jatahnya sekarang ini" — kunci yang tidak dikirim dianggap
        # kosong, sehingga menetapkan cabang otomatis mencabut wilayah dan
        # sebaliknya. Menyisakan nilai lama di kolom satunya hanya akan
        # ditolak CHECK ck_bo_users_satu_jatah di basis data.
        if "region_class" in data or "branch_codes" in data:
            kelas, kode, salah = _cek_jatah(data.get("region_class"),
                                            data.get("branch_codes"))
            if salah:
                return jsonify({"error": salah}), 400
